...
\begin{itemize}[leftmargin=1.5em, label=\textbullet]
    \item \textbf{Wdrożenie SSO}: Skonfigurowałem Authentik jako dostawcę tożsamości (IdP) w Azure, umożliwiając Single Sign-On (SSO) dla wielu aplikacji.
    \item \textbf{Integracja}: Zintegrowałem usługi przy użyciu protokołów SAML 2.0 i OAuth2 z Active Directory (LDAP), centralizując zarządzanie użytkownikami.
...
\begin{itemize}[leftmargin=1.5em, label=\textbullet]
    \item \textbf{Ocena podatności}: Przeprowadziłem kompleksowy audyt bezpieczeństwa w oparciu o OWASP Top 10, identyfikując krytyczne luki (XSS, SQLi).
    ...
    \item \textbf{Remediacja}: Opracowałem szczegółowe raporty naprawcze i wspierałem proces łatania zidentyfikowanych podatności CVE.
\end{itemize}

\cvproject{Symulacja Ataku i Obrony Sieciowej}{Home Lab}{Bettercap, Wireshark, Snort}
\begin{itemize}[leftmargin=1.5em, label=\textbullet]
    ...
    \item \textbf{Inspekcja pakietów}: Analizowałem przechwycony ruch za pomocą Wireshark w celu identyfikacji anomalii i niezaszyfrowanych poświadczeń.
...
\begin{itemize}[leftmargin=1.5em, label={}]
    ...
    \item \textbf{Web Security}: OWASP Top 10, Burp Suite, OWASP ZAP, Security Headers, koncepcje WAF.
    \item \textbf{Sieci}: TCP/IP, Model OSI, Wireshark, Nmap, VPN, DNS, DHCP, Routing (podstawy BGP).
    \item \textbf{Chmura i IAM}: Microsoft Azure, Active Directory, Authentik, OAuth 2.0, SAML, Docker, Linux (Kali, Ubuntu).
